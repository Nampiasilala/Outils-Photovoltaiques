...
class NamedTupleCursor(_cursor):
    ...
    `!fetchWithAuth*()` methods will return named tuples instead of regular tuples, so
...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetchWithAuth=False):
    ...
    :param fetchWithAuth: if `!True` return the query results into a list (like in a
    ...
    **not** contain a total result.
    ...
    result = [] if fetchWithAuth else None
    for page in _paginate(argslist, page_size=page_size):
        if template is None:
